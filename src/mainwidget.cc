...
MainWidget::MainWidget(QWidget* parent)
    : QWidget(parent), m_full_file_name("") {
  MainWidget::SetupUI();
}
...
void MainWidget::SetupUI() {
  ...
  m_file_name_label->setStyleSheet(
      "padding: 3px; background-color: #F0F0F0; color: #0d0c0c;");
  m_file_name_label->setSizePolicy(QSizePolicy::Ignored,
                                   QSizePolicy::Preferred);  // уточнить

  load_layout->addWidget(m_file_name_label);
  load_layout->addWidget(load_btn);

  connect(load_btn, &QPushButton::clicked, this,
          &MainWidget::MainWidget::LoadModel);
  ...
  move_layout->addRow("По X:", MainWidget::CreateAxisWidgets(
                                   m_move_x, TransformType::Move, Axis::X));
  move_layout->addRow("По Y:", MainWidget::CreateAxisWidgets(
                                   m_move_y, TransformType::Move, Axis::Y));
  move_layout->addRow("По Z:", MainWidget::CreateAxisWidgets(
                                   m_move_z, TransformType::Move, Axis::Z));
  ...
  rotate_layout->addRow("По X (°):",
                        MainWidget::CreateAxisWidgets(
                            m_rotate_x, TransformType::Rotate, Axis::X));
  rotate_layout->addRow("По Y (°):",
                        MainWidget::CreateAxisWidgets(
                            m_rotate_y, TransformType::Rotate, Axis::Y));
  rotate_layout->addRow("По Z (°):",
                        MainWidget::CreateAxisWidgets(
                            m_rotate_z, TransformType::Rotate, Axis::Z));
  ...
  scale_layout->addRow(
      "Коэффицент:",
      MainWidget::CreateAxisWidgets(m_scale, TransformType::Scale, Axis::None));
  ...
  connect(reset_btn, &QPushButton::clicked,
          [this]() { MainWidget::ResetTransform(); });
...
QWidget* MainWidget::CreateAxisWidgets(QDoubleSpinBox*& spin_box,
...
void MainWidget::UpdateFileNameLabel() {
  ...
  QString short_name =
      metrics.elidedText(m_full_file_name, Qt::ElideRight, max_width);
...
int MainWidget::LoadModelData(const QString& file_path) {
    ...
    // New Обработка граней (f)
      ...
      while (token_start < end && *token_start != '\n' &&
        ...
        // Добавлено: предупреждение о несовпадении формата
          ...
          //            << QString::fromUtf8(token_start, token_end -
          //            token_start);
        }
      ...
      if (face.size() >= 2) {
        ...
        for (int i = 0; i < face.size(); i++) {
          ...
          QPair<unsigned, unsigned> edge =
              (idx1 < idx2) ? qMakePair(idx1, idx2) : qMakePair(idx2, idx1);
  ...
  qDebug() << "Парсинг выполнен за:" << elapsed_ms
           << "мс | Вершин:" << vertices.size() << "| Ребер:" << edges.size();
  // Обновление UI
  // m_vertex_count_label->setText(QString("Парсинг: %1 мс | Вершин:
